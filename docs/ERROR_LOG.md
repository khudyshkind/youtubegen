...
**Причина:** SDK-таймаут 120 с (`generateWithClaude`, route.ts:180) меньше реального времени генерации длинных сценариев. `calcMaxTokens` для 25-минутного видео = 12 285 токенов; при ~100 tok/s Sonnet → ~123 с > 120 с таймаута. Retry-логика перехватывала только 529/503 overload — таймаут проходил мимо без повторной попытки.
**Кредиты:** НЕ списаны — `spendCredits` вызывается только после успешного получения текста, до которого при таймауте выполнение не доходит.
**Решение (2026-08-09):**
1. `calcTimeout(maxTokens)` — динамический SDK-таймаут: `clamp(maxTokens × 12 ms, 30s, 130s)`. Кап 130 с выбран так, чтобы две попытки + 16 с задержки = 276 с < maxDuration=300 с.
2. `isAnthropicTimeout` в `anthropic-retry.ts` — отдельная функция-детектор таймаутов (408, "timeout", "TimeoutError", "ETIMEDOUT").
3. Retry при таймауте: блок catch теперь также перехватывает `isAnthropicTimeout` и делает одну повторную попытку с 16 с задержкой.
4. `didRetry` — флаг пропуска expand-пасса если retry уже был (бюджет ~276 с, expand не влезет).
5. Chunked path: добавлен `maxRetries: 0` (было упущено), `calcTimeout(sectionMaxTokens)` вместо хардкод 240 с.
6. Сообщение об ошибке: `code: 'TIMEOUT'`, HTTP 504, текст с объяснением и напоминанием о кредитах.
**Файлы:** `src/app/api/generate/script/route.ts`, `src/lib/anthropic-retry.ts`
**Частота:** первый зафиксированный; воспроизводим системно при видео ≥20–25 мин.
