...
### [2026-08-09] Chunked-сценарий теряет сквозные факты между секциями ✅ ЗАКРЫТО (3 части)

**Симптом:** Живой прогон 20-мин видео (после снижения CHUNKED_THRESHOLD до 18): имя героя меняется (Андрей → Дима), срок брака противоречит (2 года vs 3 года), время года конфликтует (январь vs ноябрь), мотив «что-то щёлкнуло» повторён дважды почти дословно, линия изгнания задваивается.

**Причина:** Chunked-генерация работает параллельно — каждая секция независима. В промпт каждой секции передаётся только список заголовков и описаний из плана (структура), но не зафиксированные факты (имена, даты, сезон). Каждая секция изобретает их заново. Инструкция «не повторяй факты соседей» не помогает: секция не знает, какие факты фактически оказались в соседних.

**Анализ вариантов:**
- Карточка фактов (Haiku-вызов перед параллельными секциями): +3 с, параллельность сохраняется, закрывает имена/даты/сезон — **рекомендован**
- Последовательная генерация с хвостом: локальная continuity, теряет параллельность, 6 секций × 11 с = 66 с вместо 11 с
- Постобработка: для 20-мин видео ~112 с только на выход при 67 tok/s — на грани таймаута
...
**Решение (Часть 3 — расширение карточки, коммит `d7eb004`):** ✅ ЗАКРЫТО.
- Добавлен раздел OBJECTS: значимые предметы с точным визуальным описанием (цвет, размер, дефекты). Haiku инвентирует предмет, если он напрашивается по сюжету; «none» если таких нет.
- TIMELINE для storytelling расширен до помесячной шкалы всех событий от начала до финала (Фев 2023 — событие; Апр 2023 — событие; ...) — секциям запрещено добавлять новые даты/промежутки (TIMELINE RULE в system prompt).
- Для других стилей: TIMELINE остался в коротком формате.
- max_tokens карточки: 300 → 500. Размер карточки: ~100 токенов (было) → ~210 токенов (storytelling) / ~120 токенов (другие стили).

**Файлы:** `src/app/api/generate/script/route.ts`

---

### [2026-08-09] Таймаут 20-мин видео + регекс-баг isAnthropicTimeout ✅ ЗАКРЫТО

**Симптом:** Два `POST 500 /api/generate/script` с `[generate/script] Request timed out` (04:33:54 UTC и 04:36:26 UTC) — оба после деплоя `5071e83` + `d5837f5`. Видео 20 мин, одиночный вызов (до порога chunked).

**Причина № 1 — регекс-баг в `isAnthropicTimeout`:**
```
/timeout|TimeoutError|ETIMEDOUT/i.test("Request timed out")  →  false
```
«timed out» (два слова) ≠ «timeout» (одно слово). Следствия:
- В inner catch: `!isAnthropicTimeout(err)` = `true` → `throw err` без retry. Ни одного повтора.
- В outer catch: `isAnthropicTimeout(err)` = `false` → упал в generic fallback → HTTP 500 вместо 504, «Ошибка генерации сценария» вместо TIMEOUT-сообщения.

**Причина № 2 — коэффициент 12 мс/ток оказался слишком оптимистичным:**
- `calcMaxTokens(20) = 9 828 токенов`
- `calcTimeout(9 828) = min(130 000, 9 828 × 12) = 117 936 мс = 118 с`
- Таймаут сработал → API генерировал ≥ 118 с → фактическая скорость **< 83.3 tok/s** в момент инцидента.
- Точная скорость неизвестна (нет поточных данных о количестве сгенерированных токенов), верхняя граница: 9 828 / 118 с = 83.3 tok/s.

**Две строки в логах = два ручных ретрая владельца**, не код: retry-код шёл бы внутри одного запроса (118 с + 16 с + 118 с = 252 с → одна запись). Разрыв 2 мин 32 с совпадает с тем, что первый запрос завершился через ~118 с, владелец дождался ответа и нажал ещё раз.

**Кредиты:** не списаны (spendCredits — только после успеха).

**Решение (коммит следующий после `d5837f5`):**
1. `isAnthropicTimeout`: регекс исправлен на `/timeout|timed\s+out|TimeoutError|ETIMEDOUT/i` — теперь «Request timed out» корректно детектируется → retry срабатывает → на таймаут вернётся HTTP 504 с понятным сообщением.
2. `calcTimeout`: коэффициент 12 → **15 мс/ток** (≈ 67 tok/s нижняя граница вместо 83.3); обновлена таблица reference для 5/10/15/17 мин.
3. `CHUNKED_THRESHOLD` 24 → **18**: 20-мин видео теперь уходит в chunked path, где каждая секция генерирует ~750 токенов (из 4 096 max) за ~10 с — заведомо меньше любого разумного timeout.

**Файлы:** `src/lib/anthropic-retry.ts`, `src/app/api/generate/script/route.ts`
