...
### [2026-08-09] Таймаут при генерации сценария — `Request timed out` (Sentry #139627127)
...
**Причина:** SDK-таймаут 120 с (`generateWithClaude`, route.ts:180) меньше реального времени генерации длинных сценариев. `calcMaxTokens` для 25-минутного видео = 12 285 токенов; при ~100 tok/s Sonnet → ~123 с > 120 с таймаута. Retry-логика (route.ts:512-513) перехватывает только 529/503 overload — таймаут проходит мимо и уходит в outer catch без повторной попытки.
**Кредиты:** НЕ списаны — `spendCredits` вызывается только после успешного получения текста (route.ts:580), до которого при таймауте выполнение не доходит.
**Файлы:** `src/app/api/generate/script/route.ts` (строки 180, 512-513, 580), `src/lib/anthropic-retry.ts`
**Варианты фикса (не применены — разведка):**
- A. Добавить `/timeout|ETIMEDOUT/i.test(msg)` в `isAnthropicOverload` → таймаут получает 16с retry.
- B. Снизить `CHUNKED_THRESHOLD` с 30 до 20 мин — длинные видео уходят в chunked path (timeout=240с).
- C. Сделать таймаут динамическим: `timeout = max_tokens * 15` мс.
**Частота:** первый зафиксированный таймаут; вероятно одиночный, но воспроизводим при длинных видео (≥20 мин) под нагрузкой API.
