# Отчёт: 2026-08-09 — расширение карточки фактов (OBJECTS + помесячный TIMELINE)

## Что сделано (коммит `d7eb004`)

**Файл:** `src/app/api/generate/script/route.ts`

### Изменения в `buildFactsCard`

1. **OBJECTS (новый раздел)** — Haiku инвентирует предметы, которые проходят через несколько секций:
   - Просит точное визуальное описание (цвет, размер, дефекты): `"плюшевый заяц с оторванным левым ухом"`
   - Указывает секцию первого появления
   - Если нет рекуррентных предметов — пишет `"none"`

2. **TIMELINE для storytelling — помесячная шкала** — вместо «ключевые сроки» просит:
   - Все события истории с явными месяцами/годами: `"Фев 2023 — свадьба; Авг 2023 — изгнание; Дек 2023 — переезд; Фев 2024 — финал"`
   - Полное покрытие от начала до конца — секции не могут «дорисовывать» промежутки
   - Для других стилей: TIMELINE в коротком формате (без изменений)

3. **max_tokens: 300 → 500** — необходимо для расширенного TIMELINE

### Изменение в `buildSystemPrompt`

Добавлена строка после карточки:
```
...
| | Символов | Токенов |
|---|---|---|
| Старый формат | ~370 | ~100 |
| Новый (storytelling) | ~1060 | ~210 |
| Рост | +690 | +110 |

Суммарный system prompt (~820 → ~950 tok) — вплотную к порогу cache (1024 tok Sonnet); у длинных видео может активироваться.

## Тест-карточка (тема «свекровь выгнала меня», 20 мин, storytelling)

```
CHARACTERS: Наталья — главная героиня, 27 лет; Антон — муж, 31 год; Галина Ивановна — свекровь, 55 лет; Марина — лучшая подруга
TIMELINE: Фев 2023 — свадьба, переезд к свекрови; Апр 2023 — первые замечания о быте; Июн 2023 — открытая ссора на кухне; Авг 2023 — изгнание; Авг–Сен 2023 — ночи у Марины; Окт 2023 — встреча с Антоном в кафе, ультиматум; Ноя 2023 — Антон соглашается на съём квартиры; Дек 2023 — переезд в свою квартиру; Фев 2024 — финал, ровно год после свадьбы
LOCATIONS: квартира свекрови ул. Ленина, 14; кухня — место главной ссоры; квартира Марины; кафе «Ромашка»; новая однокомнатная квартира
SEASON/TIME: зима 2023 (свадьба) → лето (кульминация) → осень → зима 2024 (финал)
MOTIFS: «Пока живёшь в моём доме — живёшь по моим правилам» → section 3 only; «Антон молчал» → section 6 only
OBJECTS: детский плюшевый заяц с оторванным левым ухом и потёртым животом — уходит с ним section 4; ставит на полку section 9
```

OBJECTS и помесячный TIMELINE появились. Секции теперь не вправе вводить собственные «4 месяца» или «23 октября».

## Открытые вопросы

После следующего живого прогона: проверить, что предмет называется одинаково во всех секциях и арифметика сроков не расходится.
...
### Часть 1 — карточка фактов (коммит `cab915a`)

**Изменения в `src/app/api/generate/script/route.ts`:**

Новая функция `buildFactsCard(p, sections)` — Haiku-вызов (~2-3 с) перед `generateChunkedScript`. Промпт просит: выписать или **придумать** (если нет в плане) конкретные факты в виде структурированного блока:
```
CHARACTERS: Андрей — муж, 33 года, ...; Наталья — жена, 29 лет, ...
TIMELINE: ...
LOCATIONS: ...
SEASON/TIME: ...
MOTIFS: «что-то щёлкнуло» → section 4 only; не повторять в других секциях
```

Блок вставляется в `buildSystemPrompt` (новый опциональный параметр `factsCard?`) как `CANONICAL FACTS — follow exactly, never contradict`. Если Haiku упал — graceful fallback, секции генерируются без карточки.

Параллельность секций сохранена. Single-call путь не затронут.

### Часть 2 — разведка промпта плана (код не менялся)

**Промпт плана** (`generate/plan/route.ts:36` и `generate/script/route.ts:242` — идентичны):
- просит «1-2 sentence description of its content» — это **тема**, не **событие**
- нет требования нарративной дуги для storytelling
- нет запрета на повторение одного события в двух секциях
- нет требования «что нового происходит»

**Вероятная механика двух изгнаний:** две секции плана с близкими описаниями (финансовый кризис → переезд; ультиматум → разрыв), которые оба описывают один и тот же нарративный момент.

**Предложение по правке** (согласовано, не закодировано):
- Для storytelling: явная нарративная дуга, каждая секция = новое событие, одно событие = только одна секция
- Для всех стилей: каждая секция вводит что-то новое, чего нет в других

## Открытые вопросы владельцу

1. После живого прогона 20-мин видео — посмотреть лог `[generate/script] facts-card: ...` и убедиться, что карточка сгенерировалась.
2. Если имена/даты/сезон стали стабильными — подтвердить, что Часть 1 закрыта.
3. Правку промпта плана (Часть 2) — согласовать и запустить отдельной задачей.

---

# Отчёт: 2026-08-09 — регекс-баг isAnthropicTimeout + новые пороги

## Что сделано

### Причина (из анализа Vercel-логов)

Два лога `[generate/script] Request timed out` (04:33:54 и 04:36:26 UTC, 20-мин видео, коммиты `5071e83` + `d5837f5` уже задеплоены):

**Баг 1 — регекс `isAnthropicTimeout`:**
```
/timeout|TimeoutError|ETIMEDOUT/i.test("Request timed out")  →  false
```
«timed out» (два слова через пробел) ≠ «timeout» (одно слово). Следствия:
- inner catch: `!isAnthropicTimeout` = true → `throw apiErr1` → **retry не запустился**
- outer catch: `isAnthropicTimeout` = false → generic fallback → HTTP **500** вместо 504, «Ошибка генерации сценария» вместо TIMEOUT-сообщения

Две записи в логах — два ручных ретрая владельца: каждый запрос таймаутил за ~118 с, первый завершился ~04:35:52, владелец сразу нажал снова → 04:36:26.

**Баг 2 — коэффициент 12 мс/ток оказался оптимистичным:**
- `calcTimeout(9828) = 117 936 мс = 118 с` → таймаут → скорость API **< 83.3 tok/s** в момент инцидента.
- Точная скорость неизвестна (нет поточных данных о генерации); фактическое значение: «где-то ниже 83 tok/s».

### Изменения

**`src/lib/anthropic-retry.ts`** — регекс исправлен:
```
/timeout|timed\s+out|TimeoutError|ETIMEDOUT/i
```
«Request timed out» теперь детектируется корректно → retry срабатывает → HTTP 504 + TIMEOUT-сообщение.

**`src/app/api/generate/script/route.ts`**:
1. `calcTimeout`: коэффициент **12 → 15 мс/ток** (≈ 67 tok/s нижняя граница вместо 83.3). Обновлена reference-таблица в комментарии: 5→37 с, 10→74 с, 15→111 с, 17→125 с.
2. `CHUNKED_THRESHOLD`: **24 → 18** — 20-мин видео (9 828 токенов) теперь идёт в chunked path, где секции ~750 токенов реально → ~10 с при любой скорости API.

### Таблица после правки

| Длительность | Путь | calcTimeout | 2 попытки + 16 с | Запас @ 70 tok/s |
|---|---|---|---|---|
| 5 мин | single | 37 с | 90 с | ✓ |
| 10 мин | single | 74 с | 164 с | ✓ |
| 15 мин | single | 111 с | 238 с | ✓ |
| 17 мин | single | 125 с | 266 с < 295 с | ✓ (7.4 с) |
| 18+ мин | **chunked** | ~49 с/секция | N/A — секции ~10 с | ✓✓ |

## Изменения в файлах состояния

- ERROR_LOG: запись «Повторная ошибка» переписана как ✅ ЗАКРЫТО с подтверждённой причиной и решением
- ERROR_LOG: запись «Таймаут» (первая, закрытая) — не тронута
- CONTEXT: блок про calcTimeout и CHUNKED_THRESHOLD обновлён (15 мс/ток, 18 мин, факт < 83 tok/s)

## Открытые вопросы владельцу

Нет. Единственный вопрос был «почему HTTP 500 а не 504» — отвечен выше (регекс-баг).
При следующем таймауте: HTTP 504 + «Anthropic не успел... Кредиты не списаны.»; код 'TIMEOUT' в JSON.

---

# Разведка: 2026-08-09 — повторная «Ошибка генерации сценария»

## Инструменты доступны / недоступны

| | |
|---|---|
| Sentry | ❌ нет доступа из инструментов |
| Vercel API / logs | ❌ VERCEL_TOKEN не задан в локальном env |
| Код route.ts | ✅ прочитан полностью |
| Код enhance-guard.ts | ✅ прочитан (MIN_TOKENS = 4 096) |

## Что установлено по коду

### 1. «Ошибка генерации сценария» = outer catch fallback (не timeout, не overload)

```
if (isAnthropicOverload) → 'Нейросеть перегружена'
if (isAnthropicTimeout)  → 'Anthropic не успел...'
→ 'Ошибка генерации сценария'   ← именно это получил владелец
```

Fallback срабатывает только если ошибка не 408/timeout/529/503. Кандидаты: HTTP 400/500, сетевая ошибка без "timeout" в сообщении, неожиданная JS-ошибка.

### 2. Chunked path (≥ 24 мин) — точка уязвимости

`generateInternalPlan` (вызывается если `plan_sections` нет в запросе) не оборачивает `anthropic.messages.create` в try-catch. Любая API-ошибка, кроме таймаута и overload → outer catch → «Ошибка генерации сценария». Это реальный путь отказа.

Расчёт для 24-мин видео (чтобы убедиться, что timeouts там не проблема):
- sectionMaxTokens = max(4096, 982) = **4096** (MIN_TOKENS доминирует)
- calcTimeout(4096) = **49 152 мс**; реальная генерация 260 слов ≈ 7.5 с @ 100 tok/s ✓

### 3. Кредиты: не списаны ни в каком пути отказа

Chunked: `spendCredits` — после `generateChunkedScript !== null`.  
Single-call: `spendCredits` — после успешного `gen.text`.

## Чего не хватает для диагноза

| Вопрос | Где взять |
|---|---|
| Был ли d5837f5 задеплоен к моменту ошибки? | Vercel → Deployments → timestamp d5837f5 |
| Duration видео? | Sentry → Context `generate` → `project_id` → БД |
| Plan_sections были в запросе? | Sentry → Request payload или Vercel logs |
| Точный error class? | Sentry → Issues → Events → последнее событие → Exception |
| Новые события в #139627127 за последний час? | sentry.io/issues/139627127 → Events |

## Гипотезы по убыванию вероятности

1. **Старый деплой** — ошибка произошла до завершения деплоя d5837f5. На старом коде 25+ мин видео таймаутит со старым сообщением «Модель не ответила вовремя» (это не «Ошибка генерации сценария»), значит причина другая даже на старом коде.
2. **Нетаймаутная API-ошибка Anthropic** — Anthropic вернул 400/500, `isAnthropicTimeout` не сработал → fallback. Вероятно на любом пути.
3. **Chunked path + нет plan_sections** — `generateInternalPlan` упал с ненулевым HTTP-кодом не 408/529/503.

## Изменения в файлах состояния

- ERROR_LOG: новая запись [2026-08-09] «Повторная ошибка...» со статусом ⏳

## Открытые вопросы владельцу

1. Проверить Vercel Dashboard — когда завершился деплой d5837f5 vs время ошибки?
2. Открыть Sentry → Issues → `#139627127` → Events → найти новые события за последние ~30 мин, скопировать exception class + message + первые строки stack trace.
3. Какая длительность видео? Был ли план сгенерирован до попытки?

**Код менять нельзя до ответов на эти три вопроса.**

---

# Отчёт: 2026-08-09 — снижение CHUNKED_THRESHOLD 30→24

## Что сделано

Одно изменение: `CHUNKED_THRESHOLD = 30` → `CHUNKED_THRESHOLD = 24` в [route.ts:217](src/app/api/generate/script/route.ts#L217).

### Обоснование

Граница систематического отказа single-call path — 27 мин (`13 226 токенов → 132 с @ 100 tok/s > кэп 130 с`). 26 мин держится с 3 с запасом — хрупко. Принято решение снизить до 24, которые дают ~12 с запаса при 100 tok/s и ~21 с при 95 tok/s.

При 24 мин секции chunked path: `wordsPerSection = round(24 × 130 / 10) = 312`, `sectionMaxTokens ≈ max(MIN_TOKENS, ceil(312 × 2.9 × 1.3)) = ~1 178 токенов`, `calcTimeout(1178) = 30 000 мс`, реальная генерация ≈ 12 с → тройной запас.

### Проверка на зашитые 30

- `src/lib/` — нет других файлов с этой константой.
- `src/app/(dashboard)/` — `analytics/page.tsx` содержит «15–30 мин» как аналитический бакет, не связан с порогом.
- GPT-4o error message в route.ts использует `${CHUNKED_THRESHOLD}` динамически → обновился автоматически.

## Изменения в файлах состояния

- TASKS: пункт обновлён (добавлена пометка «+порог»)
- ERROR_LOG: запись [2026-08-09] переписана полностью — симптом, причина (хардкод при динамическом max_tokens), находка про chunked maxRetries, всё решение включая снижение порога
- CONTEXT: блок про /generate/script расширен: формула, допущение 100 tok/s, CHUNKED_THRESHOLD, calcTimeout

## Открытые вопросы владельцу

Нет. Все диапазоны теперь покрыты: 5–23 мин → single-call (запас ≥12 с), 24+ мин → chunked (секции ~30 с timeout, ~10–12 с реальное).

---

# Отчёт: 2026-08-09 — фикс таймаута /generate/script

## Что сделано

### Изменения

**`src/lib/anthropic-retry.ts`** — новая экспортируемая функция:
```typescript
export function isAnthropicTimeout(error: unknown): boolean
```
Детектирует таймауты: HTTP 408, "timeout", "TimeoutError", "ETIMEDOUT" в сообщении.

**`src/app/api/generate/script/route.ts`** — четыре точечных изменения:

1. **`calcTimeout(maxTokens): number`** — новая функция. `clamp(maxTokens × 12 мс, 30 с, 130 с)`.
   - 130 с кап: два attempt + 16 с = 276 с < maxDuration=300 с ✓
   - Передаётся в `generateWithClaude` вместо хардкода 120 с.

2. **Retry на тайм-аут** — блок catch в single-call path расширен: `!isAnthropicOverload && !isAnthropicTimeout → throw`.
   - `didRetry = true` при retry → expand-пасс пропускается (бюджет ~276 с, expand не влезет).

3. **Chunked path** — `maxRetries: 0` (было пропущено; SDK default=2 → 3 × 240 с = 720 с ≠ 300 с), `calcTimeout(sectionMaxTokens)` вместо 240 с. Клиент для `generateInternalPlan` — `timeout: 60_000, maxRetries: 0`.

4. **Сообщение об ошибке** — `code: 'TIMEOUT'`, HTTP 504, текст:  
   «Anthropic не успел сгенерировать сценарий в отведённое время. Кредиты не списаны. Попробуйте ещё раз или уменьшите длительность видео.»

### Таблица таймаутов (single-call, формула `max(2048, ceil(dur×130×2.9×1.3))`)

| Длительность | max_tokens | calcTimeout | 2 попытки + 16 с | Влезает? |
|---|---|---|---|---|
| 5 мин | 2 458 | 30 с (min) | 76 с | ✓ |
| 15 мин | 7 373 | 88.5 с | 193 с | ✓ |
| 25 мин | 12 285 | 130 с (cap) | 276 с | ✓ |
| 29 мин | 14 251 | 130 с (cap) | 276 с | ✓ |

⚠️ 29-мин видео на single-call path: 14 251 токенов @ 100 tok/s = 142.5 с > cap 130 с. Обе попытки могут таймаутнуть. Системного решения нет без снижения CHUNKED_THRESHOLD (нет по ТЗ). Retry даёт шанс, если API быстрее обычного.

### Ответ про chunked path (п. 4 задачи)

Упущение, не осознанное решение. `maxRetries` не задан → SDK default 2. На 529/503 с `timeout: 240_000`: 3 попытки × 240 с ≈ 720 с, Vercel убивает на 300 с без чистого ответа. Исправлено: `maxRetries: 0`.

## Что не изменилось

- `spendCredits` вызывается только после успешного текста — кредиты в безопасности.
- Retry на 529/503 (`isAnthropicOverload`) — не тронут.
- `CHUNKED_THRESHOLD = 30` — не тронут (п. 3 задачи).

## Изменения в файлах состояния

- TASKS: задача «Починить retry» помечена `[x]`
- ERROR_LOG: запись [2026-08-09] дополнена решением, помечена ✅ ЗАКРЫТО
- CONTEXT: добавлена запись про calcTimeout в разделе «Студия»
- WORKFLOW: добавлен кандидат про динамический SDK-таймаут

## Открытые вопросы владельцу

1. Проверить на 20–25-минутном видео в часы пиковой нагрузки — теперь должно работать.
2. 29-минутные видео могут всё ещё таймаутить (142.5 с при 100 tok/s > 130 с cap). Если появятся жалобы — вернуться к вопросу о снижении CHUNKED_THRESHOLD.

---

# Разведка: 2026-08-09 — таймаут /generate/script (Sentry #139627127)

## Что сделано

Разведка по пяти пунктам. Код не менялся.

### 1. Таймауты

- **Single-call path** (`generateWithClaude`, route.ts:180): `timeout: 120_000` — **явный SDK-таймаут 120 с**. `maxRetries: 0`.
- **Chunked path** (≥30 мин, route.ts:370): `timeout: 240_000`, `maxRetries` не задан (SDK default = 2).
- **`maxDuration` роута** (route.ts:16): 300 с.
- **Что наступает раньше:** SDK 120 с < Vercel 300 с. На single-call path SDK падает первым.
- **Для каких видео реален таймаут:** формула `duration × 130 × 2.9 × 1.3`; при 25 мин = 12 285 токенов; при ~100 tok/s Sonnet → ~123 с > 120 с. Таймаут закономерен при 20–29 мин под нагрузкой API.

### 2. Размер запроса

- `calcMaxTokens` (route.ts:168-172) — динамический. Для 5 мин = 2 458 (то же число из ERROR_LOG от 20.07), для 25 мин = 12 285. Число **не фиксировано**, изменено с 20.07 в сторону пропорционального расчёта.
- Модель по умолчанию: `claude-sonnet-4-6` (route.ts:181). Opus: `claude-opus-4-5`.

### 3. Ретраи

- `withAnthropicRetry` из `anthropic-retry.ts` в `/generate/script` **не используется** — есть инлайн-аналог (route.ts:510-519).
- Инлайн retry ловит только `isAnthropicOverload` (status 529/503 или "overloaded" в тексте).
- Таймаут (`"Request timed out"`) — **не overload** → `!isAnthropicOverload(apiErr1) → throw apiErr1` → retry не срабатывает → пользователь получает ошибку сразу.
- `maxRetries: 0` в SDK (route.ts:180) — авто-ретри SDK тоже отключён.
- **Итог: ни одного retry при таймауте.**

### 4. Кредиты

- `requireCredits` (route.ts:442) — проверка, не списание.
- `spendCredits` (route.ts:580) — вызывается только после успешного `gen.text`. При таймауте не достигается.
- **Кредиты НЕ списаны. Пользователь ничего не потерял.**

### 5. Частота

- В ERROR_LOG — первый зафиксированный таймаут (запись от 20.07 — про 529, это другое).
- По описанию — одиночный инцидент; без доступа к Sentry точное число событий неизвестно.
- **Воспроизводим системно** при видео ≥20–25 мин + замедление API.

## Что не получилось

Нет доступа к Sentry dashboard — количество событий и affected users неизвестно точно.

## Изменения в файлах состояния

TASKS:    добавлен пункт «Починить retry при таймауте /generate/script»
CONTEXT:  без изменений
WORKFLOW: без изменений
ERROR_LOG: новая запись [2026-08-09] таймаут /generate/script

## Открытые вопросы владельцу

1. **Какая длительность видео была у пользователя?** Это определит, в каком пути (single/chunked) произошёл таймаут.
2. **Какой фикс выбрать?**
   - A (мин. изменения): добавить таймаут в retry-логику — 1 строка в `isAnthropicOverload`.
   - B: снизить `CHUNKED_THRESHOLD` 30→20 мин — видео 20–29 мин уходят в chunked path (240с, parallel sections).
   - C: динамический таймаут `max_tokens × 15 мс`.
   Вариант B + A вместе = наименьший риск регрессий.
3. Стоит ли отдельно протестировать 25-минутное видео в ночное время (нагрузка API)?

---

# Отчёт: 2026-08-08 (задача 8 — починка newcomer_share: Topic-каналы + пороги квалификации)

## Что сделано

### 1. Пороги квалификации новичка (subs ≥ 100, totalViews ≥ 500)

**`src/app/api/analytics/sub-niche-finder/route.ts`:**
- Добавлены константы `NEWCOMER_MIN_SUBS = 100` и `NEWCOMER_MIN_VIEWS = 500`.
  **Обоснование порогов:**
  - 100 подписчиков — минимум для получения кастомного URL на YouTube; ниже этого нет
    смысла говорить о «реальной» аудитории.
  - 500 суммарных просмотров — эквивалент ~5 видео × 100 просм. или ~10 видео × 50 просм.
    Канал с 0–499 просмотрами либо ничего не публиковал, либо публиковал без отклика.
  Выбор именно этих порогов (не 50/200 и не 500/5000) продиктован задачей: оставить
  в метрике новичков с реальной аудиторией, не замещая их ghost-аккаунтами.

- В Step 3b `channels.list` теперь парсится `statistics.viewCount` (было только `subscriberCount`).
- Новое поле `qualifying_channels: number` в `RawEnriched` — счётчик каналов, прошедших порог.
- Условие: канал включается в знаменатель `newcomer_share` только при `sc >= 100 && vc >= 500`.
- Условие для числителя (newcomer_channel_ids): то же + `ageMonths < 12`.

### 2. Исключение Topic-каналов

**Step 3b:** если `title.endsWith(' - Topic')` → канал пропускается через `continue`.
Исключён из **всех** channel-level метрик: `channel_ages_months`, `subs`, `newcomer_share`, `newcomer_channel_ids`.

- «Speed Brake Armed - Topic» (3 подп.) и «KiLNCiTY - Topic» (2 подп.) из кроссфит-прогона
  теперь не влияют ни на `newcomer_share`, ни на `top_subs_median`.
- Topic-каналы больше не попадают в `newcomer_channel_ids` → в разбор канального уровня 4.

### 3. null вместо 0 при отсутствии qualifying-каналов

- `newcomer_share: number | null` в `ComputedNiche`, `SubNicheResult.metrics`, `SNMetric<number | null>` в page.tsx.
- Значение `null` означает: все каналы были Topic или ghost — посчитать нечего.
- Значение `0` означает: qualifying-каналы есть, но ни один не моложе 12 месяцев.

**page.tsx:** `PenetBadge({ value: number | null })` — при `null` рендерит `«—»`. Sort по `(value ?? -1)`.
**channel-breakout/route.ts:** `niche_context.newcomer_share: number | null` — в prompt выводит «нет данных» при null.

### 4. Сброс кэша — ключ v2 → v3

Старые записи посчитаны по прежней методике (без Topic-фильтра, без порогов квалификации).
Новый ключ: `...|v3`. Все запросы теперь будут пересчитаны при следующем прогоне.

### 5. Ретроспективный пересчёт по двум поднишам (расчёт из описанных данных)

Живых API-запросов не делалось. Оценки основаны на данных из задания и прогона channel-breakout.

**Кроссфит (newcomer_share = 77%):**
- Исходный расчёт: ~38–39 из ~50 каналов созданы < 12 мес.
- После исключения Topic-каналов: −2 (Speed Brake Armed, KiLNCiTY) из знаменателя и числителя.
- После порога subs ≥ 100: нишевые данные из breakout показали медиану ~84 просм./видео,
  что соответствует очень малым каналам. При range 2–400 подп. большинство находится ≤ 100.
  Оценка: ≥ 60–70% каналов отсеиваются.
- После порога views ≥ 500: каналы с медианой 84 просм./видео и ~3–5 видео имеют
  totalViews ≈ 250–420 — ниже порога. Ещё часть исключается.
- **Оценочный результат**: qualifying denominator = 8–15 каналов из 50.
  Из них реальных newcomers (< 12 мес.) — неизвестно без raw data, но вероятно 3–7.
  Новый newcomer_share: **~25–50%**, или `null` если qualifying_channels = 0.
  Ключевой сигнал: была пустота — станет «мало данных».

**Электронная танцевальная для клубов (newcomer_share = 43%):**
- Нища более живая: реальные каналы с заметной аудиторией.
- Topic-каналы (музыка — тема с высокой Topic-долей): исключается ≥ 2–5.
- Ghost-каналы с subs < 100: в более конкурентной нише таких меньше. Оценка: −20–30%.
- **Оценочный результат**: newcomer_share ≈ **30–38%** — умеренное снижение.
  Ниша остаётся «пробиваемой», но без ghost-инфляции.

## Что не получилось

Точные цифры ретроспективы получить без нового API-запроса невозможно — raw channel data
не хранится в analytics_reports (только агрегированные метрики). Предложение:
при следующем живом прогоне на тех же нишах сравнить новые значения с archived отчётами.

## Изменения в файлах состояния

TASKS:    добавлено задача 8 (newcomer_share fix), закрыто; опровергнутое — нет
CONTEXT:  раздел sub-niche-finder, инварианты — добавлено про Topic-канальный фильтр и пороги v3
WORKFLOW: добавлен кандидат «YouTube Topic-каналы и ghost-аккаунты»

## Открытые вопросы владельцу

1. Стоит ли после следующего живого прогона (кроссфит или другая ниша) сравнить
   newcomer_share v2 vs v3 — чтобы подтвердить, что порог убирает нужный шум?
2. Пороги subs=100 / views=500 — принять как константы или хочешь другие цифры?
   Это легко поменять в двух строках.

---

# Отчёт: 2026-08-08 (задача 7 — история подниш, название направления, анализ newcomer_share)

## Что сделано

### 1. История: связь breakout-отчётов с родительским отчётом ниш

**`src/app/(dashboard)/analytics/page.tsx`:**

- `HistoryTab.onOpen` теперь принимает `parent?: AnalyticsReport`
- При открытии `channel_breakout`-отчёта из истории — ищет совпадающий `sub_niche_finder`-отчёт (совпадение по `sub_niche_name` в `sub_niches[*].name`)
- Новое состояние в главной странице: `openedParentReport: AnalyticsReport | null`
- `handleOpenReport` принимает и запоминает `parent`; `clearOpenedReport` сбрасывает оба
- `SubNicheTab` получил prop `externalParentResult?: SubNicheFinderData | null`; в main JSX передаётся `openedParentReport.result`
- Внутри `SubNicheTab` — `showingParentFromHistory: boolean` + `useEffect([externalBreakout])` (сброс при смене отчёта)
- При открытии breakout из истории: если есть родитель → кнопка «Назад» ведёт к `SubNicheResultsView(parent)`, откуда «Назад» возвращает в breakout
- Квота не тратится: оба отчёта берутся из JSON истории

### 2. Название направления в заголовке отчётов

**`SubNicheResultsView`:**
- Заголовок переделан в flex-column; добавлена строка `{data.broad_niche} → {data.direction}` (полностью, переносится при необходимости)
- Показывается только если хотя бы одно из полей непустое; `direction` null-safe

**`ChannelBreakoutResultsView`:**
- Счётчик `cb_newcomer_count` вынесен вправо в отдельный row
- `🔬 {data.sub_niche_name}` стал отдельной строкой (`text-cyan-300 font-semibold`), может переноситься на несколько строк

### 3. Анализ newcomer_share: является ли метрика завышенной (без кода)

**Проблема.** `newcomer_share` = доля каналов < 12 мес в топ-50 поиска. Если в нише почти никто не снимает, топ заполняется слабыми новичками с 0–50 подписчиками → метрика показывает «легко войти», хотя на самом деле просто «никто не снимает».

**Диагностика.** Два сигнала одновременно:
- `newcomer_share` > 0.5
- `median_views_per_video` < 2000 (из sub-niche-finder L2)

Если оба истинны — высокая вероятность ложноположительного «проходимость», а не реального открытого рынка.

**Предлагаемый кодовый фикс (отдельная задача):**  
В API `sub-niche-finder` при расчёте `newcomer_share` учитывать только каналы с `>= 100 подписчиков`. Каналы с 0–99 подписчиками не являются рыночным сигналом — они не конкурируют всерьёз и не отражают реальную пробиваемость. Это одна строка в SQL / фильтр JS: `channels.filter(c => c.subscriber_count >= 100)`.

**Почему сейчас не менять:** Изменение повлияет на все старые отчёты в истории — численные значения изменятся. Лучше выкатить вместе с версионированием отчётов или одновременно с добавлением UI-предупреждения (badge «низкий спрос» при `newcomer_share > 0.5 && median_views < 2000`).

## Коммит

TBD (будет после проверки)

## Что проверить владельцу

1. Открыть из истории отчёт channel-breakout → кнопка «Назад» должна вести к родительскому отчёту поиска подниш (если он есть в истории). Второй клик «Назад» возвращает в channel-breakout.
2. Открыть любой sub-niche-finder отчёт → под кнопками навигации — строка «Тема → Направление» (полная, не обрезанная).
3. В заголовке channel-breakout → название поднишы переносится на новую строку, не обрезается.
4. Квота не уменьшается при открытии из истории (только просмотр JSON).
...
### Кнопка «Разобрать новичков»

**`src/app/(dashboard)/analytics/page.tsx`:**

- В таблице надёжных подниш и в карточках топ-5 появилась кнопка **🔬 Разобрать новичков**.
- Активна, если `newcomer_channel_ids.length >= 3`; неактивна с tooltip:
  - «Нужен новый прогон — отчёт не содержит ID каналов» — если `ids.length === 0` (старый отчёт)
  - «Недостаточно новичков (нужно минимум 3)» — если `ids.length < 3`

### Подтверждение (confirm)

- Диалог по образцу уровня 2: cyan-карточка с заголовком, описанием.
- Выбор **20 / 50** каналов (только варианты ≤ доступных); рядом — оценка квоты и времени:
  - 20 → `~41 ед. квоты · ~30 с`
  - 50 → `~101 ед. квоты · ~60 с`
- Badge с кредитами (`−2000 кр.`), счётчик доступных каналов.
- Ошибки API (byok, insufficient_credits) выводятся inline.

### `ChannelBreakoutResultsView`

1. **Нет новичков** — крупный блок с emoji 🕳️ и объяснением, не пустая таблица.
2. **Сводка** (cyan-карточка): 6 метрик — найдено каналов, набрали 1 000 до 5 мес., медиана времени / просмотров / частоты / Shorts.
3. **Таблица каналов**: название (ссылка на YT-канал), возраст, подписчики, просм./видео, публикации, разброс (×, amber если >10), доля Shorts, «До 1 000 ≤N мес.» (зелёный ≤5, жёлтый ≤9, иначе серый). Первое топ-видео — подстрочник под названием.
4. **Сноска** под таблицей: объяснение, что «До 1 000» — не более чем за N мес., т.к. API не даёт точную дату.
5. **Вывод модели** (Card с badge «оценка»): 4 аспекта + overall в cyan-блоке.

### История

- `AnalyticsReport.report_type` расширен: добавлен `'channel_breakout'`
- `AnalyticsReport.result` расширен: `ChannelBreakoutData`
- `REPORT_ICONS['channel_breakout'] = '🔬'`
- `handleOpenReport` маппит `channel_breakout` → вкладка `sub_niche`
- `SubNicheTab` принимает `externalBreakout?: ChannelBreakoutData | null` + `onClearExternalBreakout?`:
  - Если задан — показывает `ChannelBreakoutResultsView` без API-запроса

### i18n.ts

44 новых ключа `cb_*` (ru + en): все тексты, тултипы, метки, сноски.

### Типы в page.tsx

- `ChannelMetricsItem` — структура одного канала
- `ChannelBreakoutData` — полный результат route

## Коммит

`0b4f8ab` — feat(analytics): channel-breakout UI

## Что проверить владельцу

1. Запустить sub-niche-finder → в таблице появились кнопки 🔬 у каждой строки
2. Нажать «Разобрать новичков» → confirm-карточка с выбором 20/50
3. Запустить → результат: сводка + таблица + вывод модели
4. Из вкладки История → открыть `channel_breakout`-отчёт → открывается без квоты
5. Открыть старый сохранённый отчёт уровня 2 → кнопки 🔬 неактивны с подсказкой
