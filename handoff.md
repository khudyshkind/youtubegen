# Разведка: 2026-08-09 — повторная озвучка становится медленнее

## Задача — РАЗВЕДКА ONLY, код не менялся

**Симптом:** повторная озвучка одного текста с каждым разом идёт дольше.

## Выводы

### 1. Путь по движкам
- `secretvoicer`, `voicer` → **async**-путь: Vercel диспетчер (`audio/route.ts:603`) → Railway `processAudioJob` (`video-server/index.js:6652`)
- `elevenlabs`, `openai`, `google`, `apihost` → **sync**-путь, остаётся в Vercel Lambda

Дефолт и самый используемый движок — `secretvoicer` (47 операций). Симптом, вероятно, на async-пути.

### 2. Накопление на нашей стороне
- **`audio_jobs` таблица**: каждая переозвучка создаёт новую запись (video-server/index.js:5302). Нет DELETE, нет TTL, нет cron-очистки — записи накапливаются бессрочно.
- **Файлы (B2, Supabase Storage)**: перезапись по фиксированному пути — не растут.
- **Конкурентные задачи**: если нажать «переозвучить» до завершения предыдущей — оба `processAudioJob` работают параллельно и конкурируют за слоты SecretVoicer.

### 3. Накопление на стороне поставщика
- Интервал опроса фиксированный: SecretVoicer/Voicer — 2500ms (`video-server/index.js:6536`), APIHOST — 5000ms × 54 итерации (`audio/route.ts:495`). Не растёт.
- **Версия #1 (главная):** SecretVoicer (ElevenLabs-реселлер) ставит задачи в аккаунтовую очередь. При повторных запросах новые задачи ждут освобождения слота от предыдущих → наш poll видит долгий статус PENDING.

### 4. Файлы и media_expires_at
- Аудиофайл перезаписывается — нет роста.
- **Баг**: `media_expires_at` задаётся один раз от `created_at` и не обновляется при переозвучке (`video-server/index.js:2854`). Свежеперегенерированное аудио может быть помечено для удаления следующим retention-кроном (04:00 UTC), если проект старше 72ч.

### 5. Измерения
- Railway логирует chunk-время: `[audio-job:ID] chunk N/M done in X.Xs` (`video-server/index.js:6732`)
- **Как подтвердить версию:** Railway Dashboard → Logs → найти несколько `[audio-job:ID] start` + `[audio-job:ID] DONE` для одного project_id. Если растёт время chunk синтеза — провайдер медленнее. Если chunk-время постоянное, но job стартует позже — очередь на стороне провайдера.

## Основная версия
**Provider-side throttling SecretVoicer (~80%)**: при повторных запросах задачи накапливаются в очереди провайдера, каждая новая ждёт дольше. Наш poll-интервал (2.5с) фиксирован, но провайдер держит задачу в PENDING всё дольше.

## Что проверить до следующего шага
1. Railway logs — сравнить chunk-время между первым и пятым прогоном для одного проекта.
2. Убедиться, что владелец ждёт завершения перед повторным запуском (иначе concurrent-конкуренция за слоты усугубляет картину).
3. Рассмотреть фикс `media_expires_at`: обновлять поле при записи нового `audio_url` — иначе переозвученный аудио удалится по старому сроку.

---

# Отчёт: 2026-08-09 — TIMELINE RULE усилен + OBJECTS обязателен для storytelling

## Что сделано (коммит `a976f97`)

**Файл:** `src/app/api/generate/script/route.ts`

### Диагноз
- TIMELINE RULE: «не вводи новые даты» — не запрещал **вычислять**. Секция увидела Сен 2021 и Авг 2022, посчитала «три года» (неверно) и написала. Правило разрешало арифметику.
- OBJECTS пустой: промпт разрешал `"none"` — Haiku выбирала его, когда план не упоминал предмет явно.

### Изменения

**`buildFactsCard` — TIMELINE storytelling:**
Haiku теперь **предвычисляет** длительности и встраивает их прямо в шкалу:
```
Авг 2022 (11 мес. брака) — изгнание; Дек 2022 (4 мес. с изгнания) — переезд; Авг 2023 (ровно год) — финал
```
Секции видят готовые фразы и цитируют, не считают.

**`buildFactsCard` — OBJECTS storytelling:**
Убрано право писать `"none"` — Haiku инвентирует предмет обязательно: «a physical object anchors emotion». Для других стилей `"none"` по-прежнему разрешён.

**`buildSystemPrompt` — TIMELINE RULE:**
Было: «do NOT introduce any new date, duration, or time interval not listed in TIMELINE above»
Стало: «when naming a duration or time span, copy the exact phrase from TIMELINE (e.g. "11 мес. брака"). Never compute, estimate, or rephrase a duration — if it is not pre-written in TIMELINE, do not mention it.»

Правило позитивное (что делать) + запрет вычислять (не только вводить).

### Тест-карточка (тема «свекровь», 20 мин, storytelling)
```
CHARACTERS: Наталья — героиня, 27 лет; Антон — муж, 31 год; Галина Ивановна — свекровь; Марина — подруга
TIMELINE: Сен 2021 — свадьба; Мар 2022 (6 мес. брака) — ссора кухня; Авг 2022 (11 мес. брака) — изгнание; Окт 2022 (2 мес. спустя) — кафе, ультиматум; Дек 2022 (4 мес. с изгнания) — переезд; Авг 2023 (ровно год) — финал
LOCATIONS: квартира свекрови; кухня — место ссоры; квартира Марины; кафе «Ромашка»; новая квартира
SEASON/TIME: осень 2021 → лето 2022 (кульминация) → зима 2022/2023 → лето 2023
MOTIFS: «в моём доме мои правила» → section 3 only; «Антон промолчал» → section 6 only
OBJECTS: плюшевый заяц с оторванным левым ухом, выцветший розовый бантик — уходит с ним section 4; ставит на полку section 9
```
OBJECTS заполнен, TIMELINE содержит `11 мес. брака` — секция не может написать «три года».

## Открытые вопросы

После следующего живого прогона: проверить, что секции цитируют формулировки длительностей из карточки, а не считают сами.
