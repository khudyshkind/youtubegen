...
-- pg_net: async HTTP requests from triggers (requires Supabase Pro plan)
-- After enabling, set the webhook secret in SQL editor (never commit secret to git):
...
begin
  ...
  -- Requires: pg_net extension + ALTER DATABASE postgres SET app.new_user_webhook_secret = '...';
  begin
    _secret := current_setting('app.new_user_webhook_secret', true);
