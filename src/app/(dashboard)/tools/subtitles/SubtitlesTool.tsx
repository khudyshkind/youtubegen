...
export default function SubtitlesTool({ initialBlocks, initialTitle, restoredId }: Props) {
  const { t } = useLang()
  ...
  // Initialise from server-loaded restore data (or empty for a fresh page load)
  const [blocks, setBlocks] = useState<SubtitleBlock[]>(initialBlocks)
  const [duration, setDuration] = useState<number | null>(null)
  const [savedId, setSavedId] = useState<string | null>(restoredId)
  const [baseName, setBaseName] = useState(
    restoredId ? initialTitle.replace(/\.[^.]+$/, '') : 'subtitles',
  )
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      {/* Result — shown for both fresh generation and server-restored runs */}
      {hasResult && (
        <div className="mt-8">
          {/* Header row */}
