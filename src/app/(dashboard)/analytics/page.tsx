...
// ─── Plan gate block (shown when free user tries analytics — requires paid plan) ─

function PlanRequiredBlock() {
  ...
  return (
    <div className="rounded-2xl p-5 flex flex-col gap-4"
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-white mb-1">Доступно на платных тарифах</p>
          <p className="text-sm text-slate-400 leading-relaxed">Аналитика доступна начиная с тарифа Basic. Перейдите на платный план, чтобы использовать этот инструмент.</p>
        </div>
      </div>
      <button
        onClick={() => router.push('/billing')}
        className="py-2 rounded-xl text-sm font-semibold text-violet-300 transition"
...
function scheduleAnalysis(videos: { published: string; views: number; views_per_day?: number }[]) {
  ...
  const MON_SUN: number[] = [1, 2, 3, 4, 5, 6, 0]
  const dayStats = MON_SUN.map(d => {
    const vs = byDay[d] ?? []
    return { day: d, name: DAY_NAMES_RU[d] ?? '?', count: vs.length, avg: vs.length ? avg(vs) : 0 }
  })
  // All hours with ≥1 video, sorted 0..23
  const hourStats = Object.entries(byHour)
    .map(([h, vs]) => ({ hour: +h, count: vs.length, avg: avg(vs) }))
    .sort((a, b) => a.hour - b.hour)
  const bestDay  = [...dayStats].filter(d => d.count >= 3).sort((a, b) => b.avg - a.avg)[0]
    ?? [...dayStats].sort((a, b) => b.avg - a.avg)[0]
  const bestHour = [...hourStats].filter(h => h.count >= 3).sort((a, b) => b.avg - a.avg)[0]
    ?? [...hourStats].sort((a, b) => b.avg - a.avg)[0]
    ...
    <div className="flex flex-col gap-5">
      ...
      {displayResult && (
        <div className="analytics-result flex flex-col gap-5">
          ...
          {/* Duration + Caption + Engagement deep stats */}
          {displayResult.deep_stats && (() => {
            const ds = displayResult.deep_stats!
            const avgMin = Math.round(ds.avg_duration_sec / 60)
            const medMin = Math.round(ds.median_duration_sec / 60)
            const db = ds.duration_buckets
            const dbTotal = db.under5m + db.m5to15 + db.m15to30 + db.m30to60 + db.over60m
            const dbPct = (n: number) => dbTotal > 0 ? Math.round(n / dbTotal * 100) : 0
            return (
              <div className="grid sm:grid-cols-3 gap-4">
                <Card>
                  <SectionTitle>Длительность роликов</SectionTitle>
                  <p className="text-2xl font-bold text-white mb-1">{fmtDuration(ds.avg_duration_sec)}</p>
                  <p className="text-xs text-slate-500 mb-3">ср. · медиана {fmtDuration(ds.median_duration_sec)}</p>
                  <div className="flex flex-col gap-1 text-xs text-slate-400">
                    {db.under5m > 0 && <span>&lt;5 мин: {db.under5m} ({dbPct(db.under5m)}%)</span>}
                    {db.m5to15  > 0 && <span>5–15 мин: {db.m5to15} ({dbPct(db.m5to15)}%)</span>}
                    {db.m15to30 > 0 && <span>15–30 мин: {db.m15to30} ({dbPct(db.m15to30)}%)</span>}
                    {db.m30to60 > 0 && <span>30–60 мин: {db.m30to60} ({dbPct(db.m30to60)}%)</span>}
                    {db.over60m > 0 && <span>&gt;60 мин: {db.over60m} ({dbPct(db.over60m)}%)</span>}
                  </div>
                  <p className="text-xs text-slate-600 mt-2">из {ds.total_deep} роликов</p>
                </Card>
                <Card>
                  <SectionTitle>Субтитры</SectionTitle>
                  <p className="text-3xl font-bold mb-1" style={{ color: ds.caption_pct >= 50 ? '#4ade80' : ds.caption_pct > 0 ? '#facc15' : '#f87171' }}>
                    {ds.caption_pct}%
                  </p>
                  <p className="text-xs text-slate-400 leading-relaxed">
                    {ds.caption_pct === 0
                      ? 'Ни один ролик не имеет субтитров — SEO и охват снижены'
                      : ds.caption_pct < 30
                      ? 'Субтитры редкость — потенциал для роста через текст'
                      : 'Хороший охват субтитрами'}
                  </p>
                  <p className="text-xs text-violet-400 mt-2">Наш сервис добавит субтитры</p>
                </Card>
                <Card>
          ...
          {/* Year dynamics */}
          {displayResult.deep_stats?.yearly && displayResult.deep_stats.yearly.length >= 2 && (() => {
            const yearly = displayResult.deep_stats!.yearly
            const maxAvg = Math.max(...yearly.map(y => y.avg_views), 1)
            return (
              <Card>
                <SectionTitle>Динамика по годам</SectionTitle>
                <div className="flex flex-col gap-2">
                  {yearly.map(y => (
                    <div key={y.year} className="flex items-center gap-3">
                      <span className="text-xs text-slate-400 w-12 shrink-0">{y.year}</span>
                      <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
                        <div className="h-full rounded-full" style={{ width: `${Math.round(y.avg_views / maxAvg * 100)}%`, background: 'rgba(139,92,246,0.55)' }} />
                      </div>
                      <span className="text-xs text-slate-400 shrink-0 w-24 text-right">ср. {fmtNum(y.avg_views)} · {y.count} вид.</span>
                    </div>
                  ))}
                </div>
                {(() => {
                  const peak = [...yearly].sort((a, b) => b.avg_views - a.avg_views)[0]
                  const last = yearly[yearly.length - 1]!
                  if (peak && last && peak.year !== last.year && last.avg_views < peak.avg_views * 0.7) {
                    return (
                      <p className="mt-3 text-xs text-amber-400/80 border border-amber-500/20 rounded-lg px-3 py-2">
                        Пик был в {peak.year} (ср. {fmtNum(peak.avg_views)}), сейчас {fmtNum(last.avg_views)} — спад на {Math.round((1 - last.avg_views / peak.avg_views) * 100)}%.
                      </p>
                    )
                  }
                  return null
          ...
          {(() => {
            ...
            return (
              <Card>
                <SectionTitle>Динамика просмотров — {vids.length} роликов</SectionTitle>
                <ViewsChart videos={vids} />
                <p className="text-xs text-slate-500 mt-2">Наведите на бар — название и просмотры</p>
              </Card>
            )
          })()}
          ...
          {(() => {
            ...
            const { dayStats, hourStats, bestDay, bestHour } = scheduleAnalysis(vids)
              ...
              <Card>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      {dayStats.map(d => (
                        <div key={d.day} className={`flex items-center gap-2 ${d.count < 3 ? 'opacity-35' : ''}`}>
                  ...
                  <div>
                    <p className="text-xs text-slate-500 mb-2 uppercase tracking-wide">По часам (все с данными)</p>
                    {hourStats.length === 0
                      ? <p className="text-xs text-slate-600">Нет данных</p>
                      : <div className="flex flex-col gap-1.5">
                          {hourStats.map(h => (
                            <div key={h.hour} className={`flex items-center gap-2 ${h.count < 3 ? 'opacity-35' : ''}`}>
          ...
          {(() => {
            const vids = displayResult.deep_videos ?? displayResult.recent_videos
            if (!vids || vids.length < 3) return null
            const ta = titleAnalysis(vids.map(v => v.title))
            if (!ta) return null
          ...
          {/* Tag analysis (deep_stats) */}
          {displayResult.deep_stats && displayResult.deep_stats.top_tags.length > 0 && (() => {
            const ds = displayResult.deep_stats!
            return (
              <Card>
                <SectionTitle>Теги роликов (топ-20 по частоте)</SectionTitle>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {ds.top_tags.map(([tag, cnt]) => (
                    <span key={tag} className="text-xs px-2 py-0.5 rounded-full"
                      style={{ background: 'rgba(139,92,246,0.12)', color: '#c4b5fd', border: '1px solid rgba(139,92,246,0.18)' }}>
                      {tag} <span className="opacity-50">×{cnt}</span>
                    </span>
                  ))}
                </div>
          ...
          {/* Two video blocks; fallback to legacy top_videos */}
          {(displayResult.top_videos_alltime?.length || displayResult.deep_videos?.length || displayResult.recent_videos?.length) ? (
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              {(() => {
                const recentList = displayResult.deep_videos
                  ? [...displayResult.deep_videos].reverse().slice(0, 15)
                  : (displayResult.recent_videos ? [...displayResult.recent_videos].reverse().slice(0, 8) : null)
...
function RevenueTab({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const json = await res.json() as { ok: boolean; data?: RevenueResult; error?: string; code?: string }
      if (!json.ok) { if (json.code === 'plan_required') { setError('__plan__'); return }; setError(json.error ?? 'Ошибка'); return }
      setResult(json.data!)
    } catch {
      ...
      {!externalResult && (
        <Card>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {error === '__plan__' ? <PlanRequiredBlock /> : error ? <p className="text-red-400 text-sm">{error}</p> : null}
...
function CommentsTab({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!json.ok) { if (json.code === 'plan_required') { setError('__plan__'); return }; if (json.code === 'byok_required') { setError('__byok__'); return }; setError(json.error ?? 'Ошибка'); return }
      setResult(json.data!)
    } catch {
      setError('Ошибка сети')
    } finally {
      ...
      {!externalResult && (
        <Card>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {error === '__plan__' ? <PlanRequiredBlock /> : error === '__byok__' ? <ByokBlock /> : error ? <p className="text-red-400 text-sm">{error}</p> : null}
...
function RisingStarsTab({
  ...
  async function handleSearch() {
    ...
    try {
      ...
      if (!json.ok) {
        if (json.code === 'plan_required') { setError('__plan__') }
        else if (json.code === 'byok_required') { setError('__byok__') }
...
function ChannelBreakoutResultsView({
  ...
  const verdictLabels: Record<string, string> = {
    growth_speed:       t('analytics.cb_verdict_growth_speed'),
    content_cadence:    t('analytics.cb_verdict_cadence'),
    view_concentration: t('analytics.cb_verdict_spread'),
  ...
  const excludedNote = hasExcluded
    ? t('analytics.cb_excluded_note')
        ...
        .replace('{shorts}', String(s.shorts_only_count ?? 0))
    : null
  ...
  return (
    <div className="analytics-result flex flex-col gap-5">
        ...
        <div className="rounded-xl px-4 py-3 flex flex-wrap gap-4"
          ...
          <div className="flex gap-6 flex-wrap">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-white tabular-nums">{fmtNum(Math.round(data.niche_context.median_views))}</p>
            </div>
            {data.niche_context.growth_ratio !== null && (
              <div>
                <p className="text-xs text-slate-500">{t('analytics.cb_niche_growth')}</p>
                <p className="text-sm font-bold text-white tabular-nums">{data.niche_context.growth_ratio.toFixed(2)}×</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Sampling disclaimer (fix #7) — prominent, near the top */}
      {s.newcomer_count > 0 && (
        <div className="rounded-xl px-4 py-3 flex items-start gap-2"
          ...
          <p className="text-xs text-amber-200/80 leading-relaxed">{t('analytics.cb_sampling_note')}</p>
        </div>
      )}
      ...
      {s.newcomer_count > 0 && (
        <div className="rounded-2xl p-5"
          ...
          {/* Excluded note (fix #1+#2) */}
          {excludedNote && (
            <p className="text-xs text-slate-500 mt-3 pt-3 border-t border-white/5 italic">{excludedNote}</p>
          )}
        </div>
      )}

      {/* Main channels table (non-Shorts-only) */}
      {mainChannels.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-slate-300 mb-3">{t('analytics.cb_table_title')} ({mainChannels.length})</p>
          <div className="overflow-x-auto rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
            <table className="w-full text-xs">
              <thead>
                <tr style={{ background: 'rgba(255,255,255,0.04)' }}>
                  ...
                  <th className="text-right px-3 py-2.5 text-slate-500 font-medium whitespace-nowrap">
                    {t('analytics.cb_col_vtfh')}<TipIcon tip={t('analytics.cb_tip_vtfh')} />
                  </th>
                  <th className="text-right px-3 py-2.5 text-slate-500 font-medium whitespace-nowrap">
      ...
      {/* Shorts-only channels block (fix #2) */}
      {shortsOnlyChannels.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-slate-400 mb-3">{t('analytics.cb_shorts_only_title')} ({shortsOnlyChannels.length})</p>
          <div className="overflow-x-auto rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
            <table className="w-full text-xs">
              <thead>
                <tr style={{ background: 'rgba(255,255,255,0.03)' }}>
                  <th className="text-left px-3 py-2 text-slate-600 font-medium">{t('analytics.cb_col_channel')}</th>
                  <th className="text-right px-3 py-2 text-slate-600 font-medium">{t('analytics.cb_col_age')}</th>
                  <th className="text-right px-3 py-2 text-slate-600 font-medium">{t('analytics.cb_col_subs')}</th>
                </tr>
              </thead>
              <tbody>
                {shortsOnlyChannels.map((ch, i) => (
                  <tr key={i} style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td className="px-3 py-2 text-right text-slate-600 tabular-nums">{fmtNum(ch.subs)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            <div key={key} className="flex flex-col gap-0.5">
              {/* Localized label from i18n (fix #5) */}
              <p className="text-xs text-slate-500">{verdictLabels[key] ?? key}</p>
          ...
          {data.verdict.overall && (
            <div className="rounded-xl px-4 py-3 mt-1"
              ...
              <p className="text-xs text-slate-500 mb-1">{t('analytics.cb_verdict_overall')}</p>
    ...
    try {
      const res  = await fetch('/api/analytics/channel-breakout', {
        ...
        body: JSON.stringify({
          ...
          niche_median_views: breakoutNiche.metrics.median_views_per_video.value ?? 0,
          niche_context: {
            newcomer_share: breakoutNiche.metrics.newcomer_share.value,
            median_views:   breakoutNiche.metrics.median_views_per_video.value ?? 0,
            growth_ratio:   breakoutNiche.metrics.growth_ratio.value ?? null,
          },
          limit:          breakoutLimit,
          ui_lang:        uiLang,
        }),
